%% Matlab class with functions to access Bitfinex API
% Copyright by Paul Wulf (paul.wulf@web.de)
% Reference API https://docs.bitfinex.com/reference
% 
% Terminology:
% base asset refers to the asset that is the quantity of a symbol. For the symbol BTCUSDT, BTC would be the base asset.
% quote asset refers to the asset that is the price of a symbol. For the symbol BTCUSDT, USDT would be the quote asset.
classdef bitfinex_api < handle 
    ...
    properties (Access = private)
        settings; % shall be instance of class bitfinex_settings
    ...
    methods (Access = private)
        ...
        function processWSSklineMessage(parentObj, msg)
            ...
            fprintf('%s bitfinex_api::processWSSklineMessage(): Message received:\n%s\n',datetime('now'), msg);
            try
    ...
    %                     tt = unique(tt);
                ...
                fprintf('bitfinex_api::processWSSklineMessage: processing of websocket message failed with error ->%s\n received msg: %s\n', ME.identifier, msg);
            end
        ...
        function setCallBackKlines(self, clbFuncID, clbFunc, parent)
            if nargin < 4
                warning('bitfinex_api::setCallBackKlines(): not enough arguments');
                return 
            end
            clbStruct.clbFunc = clbFunc;
            clbStruct.parentObj = parent;
            if isKey(self.callback_kline_func_map, clbFuncID)
                fprintf('bitfinex_api::setCallBackKlines(): callback with ID %s already exist => replacing with new one \n', clbFuncID);
            end
        ...
        function unsetCallBackKlines(self, clbFuncID)
            if nargin < 2
                warning('bitfinex_api::unsetCallBackKlines(): not enough arguments');
                return 
            end
            if isKey(self.callback_kline_func_map, clbFuncID)
                remove(self.callback_kline_func_map, clbFuncID);
            else
                fprintf('bitfinex_api::unsetCallBackKlines(): callback ID %s not found\n', clbFuncID);
            end
        end
    ...
    methods
        ...
        function close_WSS_client(self, key)
            if ~isa(self.websocket_clients, 'containers.Map')
                warning('bitfinex_api::close_WSS_socket(): bitfinex_api::websocket_clients not an instance of containers.Map -> abort');
                return
            end
            if size(self.websocket_clients,1) == 0
                disp('bitfinex_api::close_WSS_socket(): no WSS Clients to close');
                return
            end
            if  nargin < 2
        ...
        %% bitfinex_api constructor
        % gets parameter as instance of bitfinex_settings
        function self = bitfinex_api(settings_object)
            %BINANCE_API Construct an instance of this class
            %   Detailed explanation goes here
            if isa(settings_object, "bitfinex_settings")
                ...
                ME = MException('bitfinex_api:noSuchObject', ...
                        'Passed object not instance of bitfinex_settings');
                throw(ME)
            end
            
        end
        
        %% function to get bitfinex server time
        % inputs:
        ...
        function [server_time, sever_time_formated] = get_time(self)
            ...
            try
                ...
                fprintf('get_time::Connection to bitfinex server failed with error->%s\n', ME.identifier);
                return;
            end
        ...
        %
        % outputs:
        %       symbols: array of strings with symbol pairs
        function [symbols] = get_symbols(self)
           platform_URL = self.settings.get_public_API_URL();

           try
                options = weboptions('UserAgent', 'Mozilla/5.0 (Windows NT 5.1; rv:19.0) Gecko/20100101 Firefox/19.0');
                web_ret = webread([platform_URL '/conf/pub:list:pair:exchange'], options);
                symbols = string(web_ret{1,1});
            catch ME    
                fprintf('get_symbols::Connection to bitfinex server failed with error->%s\n', ME.identifier);
                symbols = 0;
                return;
           end
        ...
        %% function to get klines/candles from bitfinex
        % inputs:
        ...
        function [time_table_klines] = get_klines(self, traiding_pair, interv, start, ende, limt)
            ...
            tt = 0;
            if nargin < 3
                disp('bitfinex_api::get_klines(): not enough arguments');
            end
            params = {};
            
            if nargin == 4
                error("bitfinex_api::get_klines(): not enough arguments");
            end

            if (nargin == 5) || (nargin == 6)
                params = {params{:}, 'start', num2str(start)};%, 'end', num2str(ende)};
            end
            if nargin == 6
                if limt > 10000, warning("bitfinex_api::get_klines(): limit >10k, set to 10k");limt=10000; end
            ...
            if isKey(self.time_table_klines, traiding_pair) && strcmp(stored_interval, interv)
                ...
                urlTemp = sprintf('%s%s:%s:t%s%s',platform_URL, '/candles/trade', interval, traidingpair, '/hist');
                options = weboptions('UserAgent', 'Mozilla/5.0 (Windows NT 5.1; rv:19.0) Gecko/20100101 Firefox/19.0');
                ...
                try
                    ...
                    fprintf('bitfinex_api::get_klines(): Retreiving traiding pair data from Bitfinex failed with->%s\n', ME.identifier);
                    %q = string({urlTemp, params{:}});a = strjoin(q, {'?', '=', '&', '=', '&', '='})
                    %fprintf('get_klines:: query string: %s\n', a);
                    return;
                end
                sz = size(data);
                date = datetime.empty(sz(1), 0);Open = zeros(sz(1), 1); Close = zeros(sz(1), 1);High = zeros(sz(1), 1);
                Low = zeros(sz(1), 1);vol = zeros(sz(1), 1);
                
                try
                    for i = 1:sz(1)
                        date(i) = datetime(data(i,1),'ConvertFrom','epochtime','TicksPerSecond',1000);%datetime(datestr(tmp{7}/86400/1000 + datenum(1970,1,1)));
                        Open(i) = data(i,2);
                        Close(i) = data(i,3);
                        High(i) = data(i,4);
                        Low(i) = data(i,5);
        ...
        %% function to open websocket to klines/candles on bitfinex
        % inputs:
        %       traiding_pair: string with supported traiding pair. Check if
        %                       traiding pair is supported by the exchange first ... 
        %                       before you request candles/klines
        ...
        function [] = ws_get_klines(self, traiding_pair, interv)
            ...
            if isKey(self.websocket_clients, traiding_pair)
                fprintf('bitfinex_api::ws_get_klines(): WSS client for traiding pair %s already exist -> replace with new one\n', traiding_pair);
        ...
        function [trade_list] = get_my_account_trade_list(self, symbol, startTime, endTime, fromID, limit)
            ...
            switch nargin
                case 1
                    query_string_from_inputs = '';
                    warning('bitfinex_api::get_my_account_trade_list(): symbol pair not set ->exit');
                    return
                case 2
                    % i kept two strings. One to calculate HMAC SHA256
                    % signature
                    if isempty(symbol)
                       warning('bitfinex_api::get_my_account_trade_list(): symbol pair is empty ->exit');
                       return
                    end
                    query_string_from_inputs = ['symbol=' char(symbol) '&'];
                    query_str_tmp = {'symbol', char(symbol)};
                case 3
                    warning('bitfinex_api::get_my_account_trade_list(): endTime argument not set->exit');
                    return
                case 4
                    if isempty(symbol)
                       warning('bitfinex_api::get_my_account_trade_list(): symbol pair is empty ->exit');
                    ...
                    if ~isempty(startTime) && ~isempty(endTime)
                        ...
                        query_str_tmp = {query_str_tmp{:}, 'startTime', num2str(startTime), 'endTime', num2str(endTime)};
                         ...
                         warning('bitfinex_api::get_my_account_trade_list(): endTime or startTime argument not double->skipping the arguments');
                    end                
                case 5
                    if isempty(symbol)
                       warning('bitfinex_api::get_my_account_trade_list(): symbol pair is empty ->exit');
                ...
                case 6
                    ...
                    if ~isempty(limit)
                        ...
                        query_str_tmp = {query_str_tmp{:}, 'limit', num2str(limit)};
                         ...
                         warning('bitfinex_api::get_my_account_trade_list(): limit argument not double->skipping the argument');
                    end                     
                otherwise
                    warning('bitfinex_api::get_my_account_trade_list(): too many arguments->skipping all of them');
                    %return
            end
        ...
        function [order_list] = get_all_my_orders(self, symbol, startTime, endTime, fromID, limit)
            ...
            switch nargin
                case 1
                    query_string_from_inputs = '';
                    warning('bitfinex_api::get_all_my_orders(): symbol pair not set ->exit');
                    return
                case 2
                    % i kept two strings. One to calculate HMAC SHA256
                    % signature
                    if isempty(symbol)
                       warning('bitfinex_api::get_all_my_orders(): symbol pair is empty ->exit');
                       return
                    end
                    query_string_from_inputs = ['symbol=' char(symbol) '&'];
                    query_str_tmp = {'symbol', char(symbol)};
                case 3
                    warning('bitfinex_api::get_all_my_orders(): endTime argument not set->exit');
                    return
                case 4
                    if isempty(symbol)
                       warning('bitfinex_api::get_all_my_orders(): symbol pair is empty ->exit');
                    ...
                    if ~isempty(startTime) || ~isempty(endTime)
                        ...
                        query_str_tmp = {query_str_tmp{:}, 'startTime', num2str(startTime), 'endTime', num2str(endTime)};
                         ...
                         warning('bitfinex_api::get_all_my_orders(): endTime or startTime argument not double->skipping the arguments');
                    end                
                case 5
                    if isempty(symbol)
                       warning('bitfinex_api::get_all_my_orders(): symbol pair is empty ->exit');
                ...
                case 6
                    ...
                    if ~isempty(limit)
                        ...
                        query_str_tmp = {query_str_tmp{:}, 'limit', num2str(limit)};
                         ...
                         warning('bitfinex_api::get_all_my_orders(): limit argument not double->skipping the argument');
                    end                     
                otherwise
                    warning('bitfinex_api::get_all_my_orders(): too many arguments->skipping all of them');
                    %return
            end
        ...
        %% function to get order book (also known as order depth) from bitfinex
        % inputs:
        %       traiding_pair: string with supported traiding pair. Check if
        %                       traiding pair is supported by the exchange first ... 
        ...
        function [order_book] = get_depth(self, traiding_pair, limit)
           ...
           try
                ...
                fprintf('get_depth::Connection to bitfinex server failed with error->%s\n', ME.identifier);
                order_book = 0;
                return;
           end
        end
        ...
        function [best_price] = get_best_price(self, traiding_pair)
           ...
           try
                ...
                fprintf('get_best_price::Connection to bitfinex server failed with error->%s\n', ME.identifier);
                best_price = 0;
                return;
           end
        end
        ...
        function [latest_price] = get_price_ticker(self, traiding_pair)
           ...
           try
               ...
               if ~isempty(traiding_pair)
                ...
                fprintf('get_price_ticker::Connection to bitfinex server failed with error->%s\n', ME.identifier);
                latest_price = 0;
                return;
           end
        end
        ...
        function [new_order_details] = place_new_order(self, symbol, side, type, newClientOrderId, cell_array_of_params)
            ...
            switch nargin
                case 1:4
                    new_order_details = [];
                    warning('bitfinex_api::place_new_order(): not enough arguments passed ->exit');
                    return
                case 5
                    % i kept two strings. One to calculate HMAC SHA256
                    % signature
                    if (isempty(symbol) || isempty(side) || isempty(type) || isempty(newClientOrderId)) 
                       new_order_details = [];
                       warning('bitfinex_api::place_new_order(): one or more input parameters are empty ->exit');
                       return
                    end
                ...
                case 6
                    ...
                    if ~isa(cell_array_of_params, 'cell')
                        warning('bitfinex_api::place_new_order(): last argument is passed to func but it is not a cell array ->exit');
                        return
                    end
                    if size(cell_array_of_params, 2) < 2
                        warning('bitfinex_api::place_new_order(): last argument is passed to func but its size < 2 ->exit');
                        return                        
                    end
                ...
                otherwise
                    new_order_details = [];
                    warning('bitfinex_api::place_new_order(): too many arguments, expected 5 arguments ->exit');
                    return
            end
            

            [server_time, ~] = self.get_time();
            fprintf('bitfinex_api::place_new_order(): server time in ms->%s\n', server_time);
            if server_time == 0
                new_order_details = 0;
                disp('bitfinex_api::place_new_order(): No server time retrieved->exiting');
                return;
            end
            ...
            try
                ...
                fprintf('bitfinex_api::place_new_order(): Placing new order failed with->%s\n', ME.identifier);
                ME
                disp('parameter dump:');
        ...
        %                           before it sends order to bitfinex: [STRATEGYID_SYMBOL_timestamp]
        ...
        function [new_order_details] = place_new_limit_buy_order(self, symbol, side, type, newClientOrderId, cell_array_of_params)
            warning('bitfinex_api::place_new_limit_buy_order(): not implemented yet');
        end
        
        %%
        %
        function [new_order_details] = place_new_limit_sell_order(self, symbol, side, type, newClientOrderId, cell_array_of_params)
            warning('bitfinex_api::place_new_limit_sell_order(): not implemented yet');
        end
        ...
        function [ret] = panic_sell_all_assets(self)
            ...
            if size(balances ,1) < 1
                warning('bitfinex_api::panic_sell_all_assets(): no assets in your portfolio -> exit');
                ret = [];
                return
            end        
